...
const GOOGLE_SCRIPT_URL: string =
  "https://script.google.com/macros/s/AKfycbxJWDL09TAfVeWmpANJj4Dyk-u2h4eedogMNSyj-GaraxPsqIXF2IoGt0ilkhmfNcPjzw/exec";
// ================================================================================================
// ================================================================================================
...
interface AppContextType {
  ...
  addOrder: (
    customerData: Omit<Customer, "id" | "registrationDate">,
    items: CartItem[],
    shippingCost: number,
  ...
  addCustomerSelection: (
    selectionData: Omit<CustomerSelection, "id" | "createdAt" | "status">
  ) => Promise<boolean>;
...
const parseData = (data: any, productOrder: string[] = []) => {
  ...
  const ordersWithRecalculatedPoints = (data.orders || []).map(
    (order: Order) => {
      ...
      if (typeof order.totalPoints !== "number" || isNaN(order.totalPoints)) {
        if (order.items && Array.isArray(order.items)) {
          order.totalPoints = order.items.reduce(
  ...
  if (productOrder.length > 0 && parsedProducts.length > 0) {
    ...
    const newProductsFromSheet = parsedProducts.filter(
      (p) => !productsInOrder.has(p.id)
    );
  ...
  return {
    ...
    orders: parseWithDates<Order>(ordersWithRecalculatedPoints).sort(
      (a, b) => b.createdAt.getTime() - a.createdAt.getTime()
    ),
    customers: parseWithDates<Customer>(data.customers || []),
    expenses: parseWithDates<Expense>(data.expenses || []).sort(
      (a, b) => b.date.getTime() - a.date.getTime()
    ),
    customerSelections: parseWithDates<CustomerSelection>(
      data.customerSelections || []
    ),
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [newlyArrivedProductIds, setNewlyArrivedProductIds] = useState<
    Set<string>
  >(() => {
    try {
      ...
      console.error(
        "Failed to parse newly arrived product IDs from localStorage",
        e
      );
      return new Set();
    }
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() =>
    loadLocalState("isAuthenticated", false)
  );
  const [exchangeRate, setExchangeRate] = useState<number>(
    FALLBACK_USD_TO_ILS_RATE
  );
  ...
  const products = useMemo(() => {
    ...
    settings.discounts,
    newlyArrivedProductIds,
  ]);

  const callGoogleScript = useCallback(
    async (action: string, payload?: any, returnFullResponse = false) => {
      setApiError(null);
      try {
        ...
        console.error(
          `Google Script API call failed for action "${action}":`,
          error
        );
        ...
        if (error.message.includes("Failed to fetch")) {
          ...
          errorMessage =
            "Failed to parse server response. The response may not be valid JSON.";
        } else {
  ...
  const removeDiscount = (productIdToDelete: string) => {
    const updatedDiscounts = (settings.discounts || []).filter(
      (d) => d.productId !== productIdToDelete
    );
  ...
  const addOrder = async (
    ...
    setIsUpdating(true);
    const result = await callGoogleScript("addOrder", {
      customerData,
      items,
      shippingCost,
      selectionId,
    ...
    if (result) {
      const newOrderWithDate = {
        ...result.newOrder,
        createdAt: new Date(result.newOrder.createdAt),
      };
      setOrders((prev) =>
        [newOrderWithDate, ...prev].sort(
          (a, b) => b.createdAt.getTime() - a.createdAt.getTime()
        )
      );
      setCustomers((prev) => {
  ...
  const updateOrder = async (
    ...
    setIsUpdating(true);
    const result = await callGoogleScript("updateOrder", {
      order: orderData,
      customer: customerData,
    });
    setIsUpdating(false);
    if (result) {
      const updatedOrderWithDate = {
        ...result.updatedOrder,
        createdAt: new Date(result.updatedOrder.createdAt),
      };
      setOrders((prev) =>
        prev.map((o) =>
          o.id === updatedOrderWithDate.id ? updatedOrderWithDate : o
        )
      );
      ...
      setCustomers((prev) =>
        prev.map((c) =>
          c.id === updatedCustomerWithDate.id ? updatedCustomerWithDate : c
        )
      );
  ...
  const updateOrderStatus = async (
    ...
    const result = await callGoogleScript("updateOrderStatus", {
      orderId,
      status,
    });
  ...
  const bulkUpdateProductPrices = async (payload: {
    ...
    setIsUpdating(true);
    const updatedProductsList: Product[] = await callGoogleScript(
    ...
    if (updatedProductsList) {
      setRawProducts((prev) => {
        const updatedProductsMap = new Map(
          updatedProductsList.map((p) => [p.id, p])
        );
  ...
  const toggleProductAvailability = async (productId: string) => {
    ...
    const result = await callGoogleScript("toggleProductAvailability", {
      productId,
    });
    setIsUpdating(false);
    if (result) {
      setRawProducts((prev) => {
        ...
        if (
          result.isAvailable &&
          productBeforeToggle &&
          !productBeforeToggle.isAvailable
        ) {
          ...
          newProductList = prev.map((p) =>
            p.id === result.productId ? updatedProduct : p
          );
        }
  ...
  const toggleProductNewlyArrived = async (productId: string) => {
    setNewlyArrivedProductIds((prevIds) => {
      ...
      try {
        ...
        console.error(
          "Failed to save newly arrived product IDs to localStorage",
          e
        );
      }
      return newIds;
  ...
  const addCustomerSelection = async (
    selectionData: Omit<CustomerSelection, "id" | "createdAt" | "status">
  ) => {
    setIsUpdating(true);
    const result = await callGoogleScript(
