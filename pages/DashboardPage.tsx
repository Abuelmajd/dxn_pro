...
const DollarSignIcon = () => (
  ...
  >
    <path
      ...
      strokeWidth={2}
      d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8v1m0 6v1m0-10a9 9 0 110 18 9 9 0 010-18z"
    />
  </svg>
);
...
const VerticalBarChart: React.FC<VerticalBarChartProps> = ({
  ...
  if (maxValue > 0) {
    ...
    tickIncrement =
      niceFractions.find((fraction) => fraction >= roughTick) || magnitude * 10;
      ...
      <div className="flex-grow flex justify-around items-end gap-2 border-l border-b border-border pl-4 relative">
        ...
        {yAxisLabels.slice(1).map((label) => (
          <div
            ...
            style={{ bottom: `${(label / effectiveMaxValue) * 100}%` }}
          ></div>
        ))}
        ...
        {data.map(({ label, value }) => (
          <div
            key={label}
            className="flex flex-col items-center w-full h-full justify-end group relative text-center"
          >
            ...
            <div
              ...
              style={{
                height:
                  value > 0 ? `${(value / effectiveMaxValue) * 100}%` : "0%",
              }}
            ></div>
    ...
    orders.forEach((order) => {
      if (!customerSpending[order.customerId]) {
        ...
        customerSpending[order.customerId] = {
          name: customer ? customer.name : t("customerNotFound"),
          total: 0,
        };
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
        <StatCard
          title={t("totalPointsThisMonth")}
          value={formatInteger(totalPointsThisMonth)}
          icon={<StarIcon />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        <ChartContainer title={t("bestSellingProducts")}>
          <VerticalBarChart
            ...
            valueFormatter={(value) =>
              `${formatInteger(value)} ${t("quantitySold")}`
            }
          />
        </ChartContainer>
