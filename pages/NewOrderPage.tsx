...
const NewOrderPage: React.FC = () => {
  ...
  const [originalSelectionId, setOriginalSelectionId] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (selection) {
      ...
      const existingCustomer = customers.find(
        (c) => c.phone === selection.customerPhone
      );
      ...
      const newCartItems: CartItem[] = selection.items
        .map((item) => {
          const productDetails = rawProducts.find(
      ...
      if (order) {
        ...
        const cartItemsWithFullData = order.items.map((item): CartItem => {
          ...
          return {
            ...
            selectedPriceType:
              item.selectedPriceType === "member" ? "member" : "normal",
          };
        });
  ...
  useEffect(() => {
    setCart((currentCart) =>
      currentCart.map((item) => {
        const basePriceToUse =
          item.selectedPriceType === "member"
            ? item.baseMemberPrice
            : item.basePrice;
    ...
    profitMargin,
    applyProfitMargin,
    cart.map((i) => i.selectedPriceType + i.productId).join(","),
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !String(customerData.whatsapp).trim() ||
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-text-primary">
            {isEditMode ? t("editInvoice") : t("createNewInvoice")}
          </h1>
        ...
        <form onSubmit={handleSubmit} className="bg-card rounded-xl shadow-lg">
          <div className="p-6 sm:p-8 space-y-6">
            <div>
              <h3 className="text-xl font-semibold text-text-primary border-b border-border pb-3 mb-4">
                {t("customerInfo")}
              </h3>
                ...
                {cart.length > 0 ? (
                  cart.map((item) => {
                    ...
                    return (
                      <div
                        key={item.productId}
                        className="flex justify-between items-start bg-card-secondary p-3 rounded-lg flex-wrap gap-4"
                      >
                        <div className="flex-grow">
                          ...
                          <div className="mt-2 space-y-2">
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                type="radio"
                                name={`priceType-${item.productId}`}
                                checked={item.selectedPriceType === "normal"}
                                onChange={() =>
                              ...
                              <span className="text-sm text-text-secondary">
                                ...
                                <span className="font-bold text-text-primary">
                                  {formatCurrency(finalNormalPrice)}
                                </span>
                              </span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                type="radio"
                                name={`priceType-${item.productId}`}
                                checked={item.selectedPriceType === "member"}
                                onChange={() =>
                                  handlePriceTypeChange(
                                    item.productId,
                                    "member"
                              ...
                              <span className="text-sm text-text-secondary">
                                {t("memberPrice")}:{" "}
                                <span className="font-bold text-text-primary">
                                  {formatCurrency(finalMemberPrice)}
                                </span>
                              </span>
                            </label>
                          </div>
                ...
                <div>
                  <label className="flex items-center gap-2 cursor-pointer mb-1">
                    <input
                      ...
                      onChange={(e) => setApplyProfitMargin(e.target.checked)}
                      className="w-4 h-4 text-accent bg-gray-100 rounded border-gray-300 focus:ring-accent"
                    />
                  ...
                  <input
                    ...
                    disabled={!applyProfitMargin}
                    className="w-full p-2 bg-input-bg rounded-md border border-border focus:ring-accent focus:border-accent disabled:bg-gray-200 dark:disabled:bg-gray-700"
                    min="0"
                    step="0.1"
                  />
                </div>
          ...
          <div className="p-6 pt-4 mt-auto border-t border-border flex-shrink-0 bg-card-secondary/50 rounded-b-xl">
            ...
            >
              {isUpdating ? (
                <>
                  <ButtonSpinner />{" "}
                  {isEditMode ? t("savingChanges") : t("creatingInvoice")}
                </>
              ) : isEditMode ? (
                t("saveChanges")
              ) : (
