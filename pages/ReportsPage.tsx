...
const ReportCard: React.FC<{ title: string; stats: FinancialStats }> = ({
  title,
  stats,
}) => {
      ...
      {hasData ? (
        <div className="space-y-3">
          ...
          <div className="flex justify-between pt-3">
            ...
            <span className={`font-bold text-lg ${netProfitColor}`}>
...
const ReportsPage: React.FC = () => {
  ...
  const monthlyPoints = useMemo(() => {
    const pointsByMonth: {
      [key: string]: { totalPoints: number; date: Date };
    } = {};
    ...
    return Object.values(pointsByMonth)
      ...
      .map((data) => ({
        month: data.date.toLocaleString(settings.language, {
          year: "numeric",
          month: "long",
          calendar: "gregory",
        }),
              ...
              {monthlyPoints.map(({ month, totalPoints }) => (
                ...
                >
                  ...
                  <span className="font-bold text-accent">
                    {formatNumber(totalPoints)} {t("points")}
                  </span>
                </li>
              ))}
            </ul>
